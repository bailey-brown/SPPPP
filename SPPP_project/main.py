...
def show_files(username):
    return (os.listdir(path='C:\\Users\\baile\\Documents\\SPPP_project\\files\\'+username))
       
app = Flask(__name__)
UPLOAD_FOLDER ='C:\\Users\\baile\\Documents\\SPPP_project\\files\\'
...
def validate():
    ...
    repeat_password=request.form['password_again']
    ...
    if user!="" and password!="" and repeat_password!="":
        if user in l:
            ...
            path=r'C:\Users\baile\Documents\SPPP_project\files\\'
            os.mkdir(path+user)
            error1="Registration Successful!"
            return render_template('login.html',error1=error1)
    else:
...
def delete():
    if request.method=='GET':
        ...
        print(choose)
        os.remove(path='C:\\Users\\baile\\Documents\\SPPP_project\\files\\'+session.get('username', None)+'\\'+(choose))
